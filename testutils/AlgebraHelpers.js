...
function test_specially_defined_mapping(op, op_name, mappings){
    QUnit.test(`${op_name} Special Specially Defined Mapping`, function (assert) {
        for (var i in mappings) {
            let input = mappings[i][0];
            let output = mappings[i][1];
            assert.deepApprox( 
                op( ...input ), output, 
                `${op_name}(inputs[${i}]) must be defined to equal outputs[${i}]`
            );
        }
    });
}
