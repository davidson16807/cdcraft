...
function SvgArrowAttributes(dependencies, settings) {
    ...
    return {
        ...
        sample: function (screen_frame_store, stored_arc, fraction) {
            ...
            const screen_arc = SamplerArcMapping(PanZoomMapping(screen_frame)).apply(resized_arc);
        ...
        head: function (screen_frame_store, stored_arc) {
            ...
            const resized_arc = sampler_arc_resizing.resize(sampler_arc, source_trim_length, -target_trim_length);
            const arrowhead_map = 
                AffineRemapping(PanZoomMapping(screen_frame)).apply(
            ...
            const screen_points = 
                cell_points.map(point => AffineMapping(arrowhead_map).position.revert(point, arrowhead_map));
